...
import {
  Loader2, ArrowRight, MailCheck, RotateCcw, CheckCircle2, XCircle,
  AlertCircle, ShieldX, User, GraduationCap, Building2, Shield,
} from "lucide-react";
...
function checkPasswordStrength(pw: string): PasswordStrength {
  ...
  const labels = ["Too weak", "Too weak", "Fair", "Good", "Strong", "Very strong"];
  const colors = ["text-red-500", "text-red-500", "text-orange-500", "text-yellow-500", "text-green-500", "text-green-500"];
  const bgColors = ["bg-red-500", "bg-red-500", "bg-orange-500", "bg-yellow-500", "bg-green-500", "bg-green-500"];
  return { score, label: labels[score] ?? "Too weak", color: colors[score] ?? "text-red-500", bgColor: bgColors[score] ?? "bg-red-500", checks };
}
...
export default function RegisterPage() {
  ...
  const isBlocked = prefillData?.notInDatabase === true || (prefillData?.exists && !prefillData.feeCleared);
  const canSubmit = prefillData?.exists && prefillData.feeCleared && !prefillData.alreadyActive;
  ...
  const submitForm = async (e: React.FormEvent) => {
    ...
    if (pwStrength.score < 5) {
      toast.error("Please choose a stronger password. All 5 requirements must be met.");
      return;
    }
    if (password !== confirm) {
    ...
    try {
      ...
      if (code === "NOT_IN_DATABASE") {
        toast.error("Your registration number is not in the KU student database.");
          ...
          {step === "form" ? (
            <Card className="border-border/70 shadow-lg">
              <CardHeader>
                ...
                <CardDescription>
                  Enter your KU registration number. Your details will be verified and auto-filled from the university database.
                </CardDescription>
              </CardHeader>
              ...
              <CardContent>
                <form onSubmit={submitForm} className="space-y-5">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      <Input
                        ...
                        className={regNumberError ? "border-destructive pr-9" : "pr-9"}
                        required
                      />
                      {prefillLoading && (
                    ...
                    <p className="text-xs text-muted-foreground">
                      Format: <span className="font-mono">J31/4338/2022</span> or <span className="font-mono">J31S/4338/2022</span>
                    </p>
                  </div>
                  ...
                  <AnimatePresence mode="wait">
                    {prefillData && (
                      <motion.div
                        key={prefillData.notInDatabase ? "not-found" : prefillData.feeCleared ? "eligible" : "fee-blocked"}
                      ...
                      >
                        {prefillData.notInDatabase ? (
                          <div className="flex items-start gap-3 rounded-lg border border-destructive/40 bg-destructive/10 px-4 py-3 text-sm">
                            ...
                            <div>
                              <p className="font-semibold text-destructive">Not in the student database</p>
                              <p className="text-muted-foreground mt-0.5">
                                This registration number was not found in KU records. Please contact the <strong>Registrar's Office</strong> or verify the number and try again.
                              </p>
                            </div>
                          </div>
                          ...
                          <div className="flex items-start gap-3 rounded-lg border border-chart-3/40 bg-chart-3/10 px-4 py-3 text-sm">
                            ...
                            <div>
                              ...
                              <p className="text-muted-foreground mt-0.5">
                                An account for this registration number is already registered.{" "}
                          ...
                          <div className="flex items-start gap-3 rounded-lg border border-destructive/40 bg-destructive/10 px-4 py-3 text-sm">
                            ...
                            <div>
                              ...
                              <p className="text-muted-foreground mt-0.5">
                                Outstanding balance: <strong>KES {prefillData.feeBalance?.toLocaleString()}</strong>. You must clear all fees at the Finance Office before you can register to vote.
                              </p>
                            </div>
                          </div>
                        ) : (
                          <div className="flex items-start gap-3 rounded-lg border border-green-500/40 bg-green-500/10 px-4 py-3 text-sm">
                            ...
                            <div>
                              <p className="font-semibold text-green-700 dark:text-green-300">Eligible to register</p>
                              <p className="text-muted-foreground mt-0.5">
                                Fee balance cleared. Your details have been verified and pre-filled below.
                              </p>
                            </div>
                          </div>
                        )}
                    ...
                    {prefillData?.exists && prefillData.feeCleared && !prefillData.alreadyActive && (
                      ...
                      >
                        <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
                          <User className="h-3.5 w-3.5" /> Student Details (from KU database)
                        </p>

                        <div className="grid gap-3 sm:grid-cols-2 text-sm">
                          <div>
                            <span className="text-muted-foreground text-xs">Full Name</span>
                            <p className="font-medium mt-0.5">{prefillData.name}</p>
                          </div>
                          <div>
                            <span className="text-muted-foreground text-xs">Gender</span>
                            <p className="font-medium mt-0.5 capitalize">{prefillData.gender}</p>
                          </div>
                          <div>
                            <span className="text-muted-foreground text-xs">University Email</span>
                            <p className="font-medium mt-0.5 break-all">{prefillData.suggestedEmail}</p>
                          </div>
                          <div>
                            <span className="text-muted-foreground text-xs">Year of Study</span>
                            <p className="font-medium mt-0.5">Year {prefillData.yearOfStudy}</p>
                          </div>
                        </div>

                        <p className="text-xs text-muted-foreground flex items-center gap-1">
                          <Shield className="h-3 w-3" />
                          These details are pulled directly from the KU Registrar's database and cannot be changed here.
                        </p>
                      </motion.div>
                    )}
                  </AnimatePresence>
                      ...
                      >
                        <div className="space-y-2">
                          ...
                          <Input
                            ...
                            required
                            value={password}
                            onChange={(e) => { setPassword(e.target.value); setShowPasswordChecks(true); }}
                          ...
                          {showPasswordChecks && password.length > 0 && (
                            <ul className="grid grid-cols-1 gap-1 sm:grid-cols-2">
                              {pwStrength.checks.map((c) => (
                                <li key={c.label} className={`flex items-center gap-1.5 text-xs ${c.passed ? "text-green-600 dark:text-green-400" : "text-muted-foreground"}`}>
                                  {c.passed
                        ...
                        <div className="space-y-2">
                          ...
                          <Input
                            ...
                            className={confirm.length > 0 && confirm !== password ? "border-destructive" : ""}
                          />
            ...
            <Card className="border-border/70 shadow-lg">
              <CardHeader>
                ...
                <CardDescription>
                  ...
                  <span className="font-medium text-foreground">{registeredEmail || prefillData?.suggestedEmail}</span>
                </CardDescription>
              </CardHeader>
              <CardContent>
                <form onSubmit={submitOtp} className="space-y-6">
                  ...
                  {devOtpHint && (
                    <div className="rounded-md border border-dashed border-chart-3/40 bg-chart-3/10 p-3 text-center text-xs text-muted-foreground">
                      ...
                      <span className="font-mono font-bold text-foreground">{devOtpHint}</span>
                    </div>
                  )}
