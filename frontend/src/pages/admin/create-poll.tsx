...
export default function AdminCreatePollPage() {
  ...
  const positionsQ = useListPositions();
  ...
  const addSrcTemplate = () => {
    const roles: string[] = (positionsQ.data as any)?.srcRoles ?? [];
    setSeats(roles.map((r) => ({ code: r.toLowerCase().replace(/[^a-z0-9]+/g, "-"), label: r, scope: "src", scopeRefId: "", gender: "" })));
    toast.success("Loaded the 9 SRC roles");
  };
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    const cleaned = seats.map((s, i) => ({
      ...
      label: s.label,
      scope: s.scope,
      scopeRefId: s.scope === "school" || s.scope === "department" || s.scope === "hostel" ? s.scopeRefId || null : null,
    ...
    try {
      await create.mutateAsync({ data: { title, description, startDate: new Date(start).toISOString(), endDate: new Date(end).toISOString(), seats: cleaned } as any });
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">Define the title, voting window and seats students will vote on.</p>
      </div>
        ...
        <Card>
          ...
          <CardContent className="grid gap-4 md:grid-cols-2">
            <div className="space-y-2 md:col-span-2">
              <Label>Title</Label>
              <Input required value={title} onChange={(e) => setTitle(e.target.value)} placeholder="KUSA SRC Elections 2026" />
            </div>
            <div className="space-y-2 md:col-span-2">
          ...
          <CardHeader>
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                ...
                <CardDescription>Each seat will appear as a separate ballot question.</CardDescription>
              </div>
              <div className="flex gap-2">
                <Button type="button" variant="outline" size="sm" onClick={addSrcTemplate} className="gap-1"><Layers className="h-3.5 w-3.5" /> Use SRC template</Button>
                <Button type="button" size="sm" onClick={add} className="gap-1"><Plus className="h-3.5 w-3.5" /> Add seat</Button>
              </div>
            </div>
          </CardHeader>
            ...
            {seats.map((s, i) => (
              <div key={i} className="grid gap-3 rounded-lg border border-border p-4 md:grid-cols-12">
                <div className="space-y-1.5 md:col-span-4">
                  <Label>Label</Label>
                  <Input required value={s.label} onChange={(e) => update(i, { label: e.target.value })} placeholder="Chairperson" />
                </div>
                ...
                <div className="space-y-1.5 md:col-span-3">
                  ...
                  {s.scope === "school" ? (
                    <Select value={s.scopeRefId} onValueChange={(v) => update(i, { scopeRefId: v })}>
                      ...
                      <SelectContent>
                        {schools.map((x: any) => <SelectItem key={x.id} value={x.id}>{x.name}</SelectItem>)}
                      </SelectContent>
                    </Select>
                  ) : s.scope === "department" ? (
                    <Select value={s.scopeRefId} onValueChange={(v) => update(i, { scopeRefId: v })}>
                      <SelectTrigger><SelectValue placeholder="All / pick" /></SelectTrigger>
                      <SelectContent>
                        {allDepartments.map((x) => <SelectItem key={x.id} value={x.id}>{x.name}</SelectItem>)}
                      </SelectContent>
                    </Select>
                  ) : s.scope === "hostel" ? (
                    <Select value={s.scopeRefId} onValueChange={(v) => update(i, { scopeRefId: v })}>
                      <SelectTrigger><SelectValue placeholder="All / pick" /></SelectTrigger>
                      <SelectContent>
                        {hostels.map((x: any) => <SelectItem key={x.id} value={x.id}>{x.name}</SelectItem>)}
                      </SelectContent>
                    </Select>
                  ) : (
