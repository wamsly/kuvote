...
export async function createPoll(req: Request, res: Response) {
  const { title, description, pollType, startDate, endDate, seats } = (req.body ?? {}) as {
    title: string;
    description?: string;
    pollType?: string;
    startDate: string;
...
export async function deletePoll(req: Request, res: Response) {
  ...
  await db.delete(candidateDocumentsTable).where(
    eq(candidateDocumentsTable.candidateId, 
      db.select({ id: candidatesTable.id }).from(candidatesTable).where(eq(candidatesTable.pollId, pollId)).limit(1) as any
    )
  ).catch(() => {});
...
export async function openApplicationWindow(req: Request, res: Response) {
  const { pollId } = req.params;
  const { timerDurationMinutes } = (req.body ?? {}) as { timerDurationMinutes?: number };
...
export async function closeApplicationWindow(req: Request, res: Response) {
  ...
  if (existing[0]) {
    await db
      .update(electionApplicationSettingsTable)
      .set({ isOpen: false, closeAt: now, closedBy: req.user!.id, updatedAt: now })
      .where(eq(electionApplicationSettingsTable.pollId, pollId));
  } else {
    await db.insert(electionApplicationSettingsTable).values({
      pollId,
...
export async function getUsers(req: Request, res: Response) {
  ...
  const filtered = genderFilter && genderFilter !== "all"
  ...
  const turnout = await Promise.all(
    polls.map(async (p) => {
      ...
      const voterIds = await db
        .selectDistinct({ userId: ballotTokensTable.userId })
        .from(ballotTokensTable)
        .where(and(eq(ballotTokensTable.pollId, p.id), eq(ballotTokensTable.used, true)));
      const votedSet = new Set(voterIds.map((v) => v.userId));
      const totalEligible = allStudents.length;
      const totalVoted = voterIds.length;

      const bySchool: Record<string, { schoolName: string; eligible: number; voted: number }> = {};
      ...
      return {
        ...
        totalVoted,
        turnoutPercent: totalEligible > 0 ? Math.round((totalVoted / totalEligible) * 1000) / 10 : 0,
        bySchool: Object.entries(bySchool).map(([id, v]) => ({
          schoolId: id,
          schoolName: v.schoolName,
          eligible: v.eligible,
          voted: v.voted,
          turnoutPercent: v.eligible > 0 ? Math.round((v.voted / v.eligible) * 1000) / 10 : 0,
        })),
      };
    }),
  );
...
export async function getVoterParticipationReport(_req: Request, res: Response) {
  ...
  const report = await Promise.all(
    voters.map(async (v) => {
      ...
      return {
        ...
        participationRate: polls.length > 0 ? Math.round((totalVoted / polls.length) * 1000) / 10 : 0,
        votingActivity,
      };
    }),
  );
...
export async function getRejectedCandidatesReport(_req: Request, res: Response) {
  const rows = await db
    .select({
      id: candidatesTable.id,
      pollId: candidatesTable.pollId,
